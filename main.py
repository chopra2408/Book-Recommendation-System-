...
raw_document = TextLoader("tagged_description.txt", "utf-8").load()
...
if os.path.exists(persist_directory):
    ...
    db_books = Chroma(
        ...
        embedding_function=embedding_model,  # Pass the embedding model directly here
    )
else:
    print("Creating a new Chroma vector database...")
    db_books = Chroma.from_documents(
        documents=documents,
        embedding=embedding_model,  # Use the embedding model directly
        persist_directory=persist_directory,
    )
...
def retrieve_semantic_recommendations(
    ...
    books_list = [int(rec[0].page_content.strip('"').split()[0]) for rec in recs]  # Access the document with rec[0]
